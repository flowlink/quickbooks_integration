  ...
  module Service
    class PurchaseOrder < Base
      ...
      def build(new_purchase_order)
        ...
        if vendor_id = purchase_order["quickbooks_vendor_id"]
          vendor = vendor_service.find_by_id vendor_id
          new_purchase_order.vendor_id = vendor.id
        else
          vendor_name = check_for_param("quickbooks_vendor_name")
          vendor = vendor_service.find_by_name vendor_name
          new_purchase_order.vendor_id = vendor.id
        end

        account_name = check_for_param("quickbooks_account_name")
      ...
      def check_for_param(name)
        purchase_order[name] || config.fetch(name)
      end
